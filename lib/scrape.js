...
let url_mhy = {
    refferer_url: `https://webstatic-sea.mihoyo.com/ys/event/signin-sea/index.html`,
    reward_url: `https://hk4e-api-os.mihoyo.com/event/sol/home`,
    role_url: `https://api-os-takumi.mihoyo.com/binding/api/getUserGameRolesByCookie?game_biz=`,
...
async function GIGetListReward(cookie) {
    const { data: html } = await axios.get(url_mhy.reward_url, {
        headers: {
            'User-Agent': "Mozilla/5.0 (iPhone; CPU iPhone OS 14_0_1 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E150",
            'Referer': `https://webstatic-sea.mihoyo.com/ys/event/signin-sea/index.html?act_id=${param_mhy.act_id}`,
...
async function GIGetInformationUser(cookie) {
    const { data: html } = await axios.get(url_mhy.role_url, {
        headers: {
            'User-Agent': "Mozilla/5.0 (iPhone; CPU iPhone OS 14_0_1 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E150",
            'Referer': `https://webstatic-sea.mihoyo.com/ys/event/signin-sea/index.html?act_id=${param_mhy.act_id}`,
            "Accept-Encoding": "gzip, deflate, br",
...
async function GIClaimReward(cookies) {
    const { data: html } = await axios(url_mhy.sign_url, {
        method: 'POST',
        headers: {
            'User-Agent': "Mozilla/5.0 (iPhone; CPU iPhone OS 14_0_1 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E150",
            'Referer': `https://webstatic-sea.mihoyo.com/ys/event/signin-sea/index.html?act_id=${param_mhy.act_id}`,
        ...
        data: {
            'act_id': param_mhy.act_id
        }
    })
    return html;
}

module.exports = { igstalk, getInstagramReels, getInstagramVideo, getInstagramImage, getTikTokMedia, getJOOXMusic, GIClaimReward, GIGetInformationUser, GIGetListReward }