...
let handler = async(m, { conn, text, usedPrefix, command}) => {
    let [q, n] = text.split `|`
    if (!q) return m.reply(`Ketik ${usedPrefix + command} <teks | author>`)
    ...
    if (res.status !==200) return m.reply('Server error')
    let json = await res.json()
    await conn.sendFile(m.chat, json.result, 'q.jpg', 'Thanks to @Nurutomo for this command:)', m)
}
